...
class BrandPrice:
    ...
    def file_to_dictionary(self, key_column: int,
                           value_column: int) -> dict[str, list[int]]:
        """
        Read from csv file named self. filename using csv.reader() method.
    ...
    def brand_by_price_d(self) -> dict[str, list[int]]:
        """
        Create and returns a dictionary whose keys are brands of cars.
    ...
    def average_selling_price_by_brand(self) -> dict[str, float]:
        """
        Get dictionary of brand name and list of selling prices by calling.
